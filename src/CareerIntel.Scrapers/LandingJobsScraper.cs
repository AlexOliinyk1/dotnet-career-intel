...
/// <summary>
/// Scrapes .NET job listings from Landing.jobs — a European tech job board
/// headquartered in Lisbon, focused on connecting developers with EU tech companies.
/// Salaries are displayed in EUR. Supports both remote and on-site positions.
/// </summary>
public sealed class LandingJobsScraper(HttpClient httpClient, ILogger<LandingJobsScraper> logger, ScrapingCompliance? compliance = null)
    : BaseScraper(httpClient, logger, compliance)
{
    private const string BaseUrl = "https://landing.jobs";
    ...
    public override string PlatformName => "Landing.jobs";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            for (var page = 1; page <= maxPages; page++)
            {
                ...
                var url = $"{BaseUrl}/jobs?q={Uri.EscapeDataString(keywords)}&page={page}";

                logger.LogInformation("[Landing.jobs] Scraping page {Page}: {Url}", page, url);

                var document = await FetchPageAsync(url, cancellationToken);
                if (document is null)
                {
                    logger.LogWarning("[Landing.jobs] Failed to fetch page {Page}", page);
                    break;
                }
                ...
                var jobCards = SelectNodes(document,
                    ...
                    "//div[contains(@class, 'listing')]//a[contains(@href, '/jobs/')] | //div[contains(@class, 'offer')]")
                    ?? SelectNodes(document,
                    "//li[contains(@class, 'job')] | //div[contains(@class, 'vacancy')]")
                    ?? SelectNodes(document,
                    "//a[contains(@href, '/jobs/') and not(contains(@href, '?'))]");

                if (jobCards is null or { Count: 0 })
                {
                    logger.LogDebug("[Landing.jobs] No job cards found on page {Page}. Structure may have changed.", page);
                    break;
                }

                logger.LogInformation("[Landing.jobs] Page {Page}: found {Count} potential listings", page, jobCards.Count);
        ...
        {
            logger.LogError(ex, "[Landing.jobs] Scraping failed");
        }

        logger.LogInformation("[Landing.jobs] Scraped {Count} .NET vacancies", vacancies.Count);
        return vacancies;
    }
    ...
    private JobVacancy? ParseJobCard(HtmlNode card)
    {
        try
        {
            ...
            if (fullUrl.Contains("?q=") || fullUrl.EndsWith("/jobs") || fullUrl.EndsWith("/jobs/"))
                return null;
            ...
            var salaryNode = card.SelectSingleNode(
                ".//*[contains(@class, 'salary')] | .//*[contains(@class, 'compensation')] | .//*[contains(@class, 'range')]");
    ...
    private JobVacancy? ParseDetailPage(HtmlDocument document, string url)
    {
        try
        {
            ...
            var skillNodes = SelectNodes(document,
                "//*[contains(@class, 'skill')]//span | //*[contains(@class, 'tag')]//a | //*[contains(@class, 'tech')]//li | //*[contains(@class, 'skill')]//li");
            var skills = new List<string>();
            if (skillNodes is not null)
            {
                foreach (var node in skillNodes)
            ...
            return new JobVacancy
            {
                ...
                Title = title,
                Company = string.IsNullOrWhiteSpace(company) ? string.Empty : company,
                City = location?.Trim() ?? string.Empty,
                Country = DetectCountry(location),
                Description = description,
                Url = url,
                SalaryMin = salaryMin,
    ...
    private static string DetectCountry(string? location)
    {
        ...
        if (lower.Contains("portugal") || lower.Contains("lisbon") || lower.Contains("porto")) return "Portugal";
        if (lower.Contains("germany") || lower.Contains("berlin") || lower.Contains("munich")) return "Germany";
        if (lower.Contains("spain") || lower.Contains("madrid") || lower.Contains("barcelona")) return "Spain";
        if (lower.Contains("france") || lower.Contains("paris")) return "France";
        if (lower.Contains("netherlands") || lower.Contains("amsterdam")) return "Netherlands";
        if (lower.Contains("uk") || lower.Contains("london")) return "UK";
        if (lower.Contains("ireland") || lower.Contains("dublin")) return "Ireland";
