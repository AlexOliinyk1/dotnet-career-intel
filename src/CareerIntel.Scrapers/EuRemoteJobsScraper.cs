...
/// <summary>
/// Scrapes job listings from EuRemoteJobs.com — a remote job board focused on European opportunities.
/// EuRemoteJobs features remote positions with EU-friendly time zones and work arrangements.
/// All positions are 100% remote with emphasis on European companies and talent.
/// </summary>
public sealed class EuRemoteJobsScraper(HttpClient httpClient, ILogger<EuRemoteJobsScraper> logger, ScrapingCompliance? compliance = null)
    : BaseScraper(httpClient, logger, compliance)
{
    private const string BaseUrl = "https://euremotejobs.com";

    // Search URLs for .NET/developer jobs
    private static readonly string[] SearchUrls =
    [
        "https://euremotejobs.com/jobs",
        "https://euremotejobs.com/jobs?category=software-development",
        "https://euremotejobs.com/jobs?category=backend"
    ];

    public override string PlatformName => "EuRemoteJobs";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var searchUrl in SearchUrls)
        {
            ...
            for (var page = 1; page <= maxPages; page++)
            {
                ...
                logger.LogInformation("[EuRemoteJobs] Scraping {Url} page {Page}", searchUrl, page);

                var document = await FetchPageAsync(url, cancellationToken);
                if (document is null)
                {
                    logger.LogWarning("[EuRemoteJobs] Failed to fetch page {Page} from {Url}", page, searchUrl);
                    break;
                }
                ...
                {
                    logger.LogWarning("[EuRemoteJobs] No job cards found on page {Page}. Trying alternative selectors...", page);
                    ...
                    if (jobCards is null or { Count: 0 })
                    {
                        logger.LogWarning("[EuRemoteJobs] Still no job cards found. Stopping at page {Page}", page);
                        break;
                    }
                }

                logger.LogInformation("[EuRemoteJobs] Found {Count} job cards on page {Page}", jobCards.Count, page);
                ...
                {
                    logger.LogDebug("[EuRemoteJobs] No next page button found, stopping at page {Page}", page);
                    break;
                }
            }
        }

        logger.LogInformation("[EuRemoteJobs] Scraped {Count} unique vacancies total", allVacancies.Count);
    ...
    private JobVacancy? ParseJobCard(HtmlNode card)
    {
        try
        {
            ...
            var linkNode = card.SelectSingleNode(".//a[@href and contains(@href, '/jobs/')]") ??
                          card.SelectSingleNode(".//a[@href and contains(@href, '/job/')]") ??
            ...
            if (!idMatch.Success)
                idMatch = Regex.Match(fullUrl, @"/job/([a-zA-Z0-9\-]+)");
            ...
            var locationNode = card.SelectSingleNode(".//*[contains(@class, 'location')]") ??
                             ...
                             card.SelectSingleNode(".//*[contains(text(), 'Europe')]") ??
            ...
            return new JobVacancy
            {
                ...
                RemotePolicy = RemotePolicy.FullyRemote, // EuRemoteJobs is remote-only
    ...
    private JobVacancy? ParseDetailPage(HtmlDocument document, string url)
    {
        try
        {
            ...
            if (!idMatch.Success)
                idMatch = Regex.Match(url, @"/job/([a-zA-Z0-9\-]+)");
        ...
        {
            logger.LogError(ex, "[EuRemoteJobs] Failed to parse detail page: {Url}", url);
            return null;
        }
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(location)) return "Europe";
