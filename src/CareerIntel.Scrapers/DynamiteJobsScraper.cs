...
/// <summary>
/// Scrapes job listings from DynamiteJobs.com — a curated remote job board.
/// DynamiteJobs focuses on high-quality remote positions for experienced professionals.
/// All positions are 100% remote with competitive compensation.
/// </summary>
public sealed class DynamiteJobsScraper(HttpClient httpClient, ILogger<DynamiteJobsScraper> logger, ScrapingCompliance? compliance = null)
    : BaseScraper(httpClient, logger, compliance)
{
    private const string BaseUrl = "https://dynamitejobs.com";

    // Search URLs for .NET/developer jobs
    private static readonly string[] SearchUrls =
    [
        "https://dynamitejobs.com/remote-jobs/software-engineering",
        "https://dynamitejobs.com/remote-jobs/back-end-development",
        "https://dynamitejobs.com/remote-jobs/full-stack-development"
    ];

    public override string PlatformName => "DynamiteJobs";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var searchUrl in SearchUrls)
        {
            ...
            for (var page = 1; page <= maxPages; page++)
            {
                ...
                logger.LogInformation("[DynamiteJobs] Scraping {Url} page {Page}", searchUrl, page);

                var document = await FetchPageAsync(url, cancellationToken);
                if (document is null)
                {
                    logger.LogWarning("[DynamiteJobs] Failed to fetch page {Page} from {Url}", page, searchUrl);
                    break;
                }
                ...
                {
                    logger.LogWarning("[DynamiteJobs] No job cards found on page {Page}. Trying alternative selectors...", page);
                    ...
                    if (jobCards is null or { Count: 0 })
                    {
                        logger.LogWarning("[DynamiteJobs] Still no job cards found. Stopping at page {Page}", page);
                        break;
                    }
                }

                logger.LogInformation("[DynamiteJobs] Found {Count} job cards on page {Page}", jobCards.Count, page);
                ...
                {
                    logger.LogDebug("[DynamiteJobs] No next page button found, stopping at page {Page}", page);
                    break;
                }
            }
        }

        logger.LogInformation("[DynamiteJobs] Scraped {Count} unique vacancies total", allVacancies.Count);
    ...
    private JobVacancy? ParseJobCard(HtmlNode card)
    {
        try
        {
            ...
            {
                logger.LogDebug("[DynamiteJobs] Job card has no link, skipping");
                return null;
            }
            ...
            if (!idMatch.Success)
                idMatch = Regex.Match(fullUrl, @"/jobs/([a-zA-Z0-9\-]+)");
            ...
            {
                logger.LogDebug("[DynamiteJobs] Job card has no title, skipping");
                return null;
            }
            ...
            var salaryNode = card.SelectSingleNode(".//*[contains(@class, 'salary')]") ??
                           ...
                           card.SelectSingleNode(".//*[contains(@class, 'pay')]") ??
            ...
            return new JobVacancy
            {
                ...
                RemotePolicy = RemotePolicy.FullyRemote, // DynamiteJobs is remote-only
        ...
        {
            logger.LogDebug(ex, "[DynamiteJobs] Failed to parse job card");
            return null;
        }
    }
    ...
    private JobVacancy? ParseDetailPage(HtmlDocument document, string url)
    {
        try
        {
            ...
            if (!idMatch.Success)
                idMatch = Regex.Match(url, @"/jobs/([a-zA-Z0-9\-]+)");
            ...
            var salaryNode = SelectSingleNode(document, "//*[contains(@class, 'salary')]") ??
                           ...
                           SelectSingleNode(document, "//*[contains(@class, 'pay')]");
            var (salaryMin, salaryMax, currency) = ParseSalaryRange(ExtractText(salaryNode));
        ...
        {
            logger.LogError(ex, "[DynamiteJobs] Failed to parse detail page: {Url}", url);
            return null;
        }
    }
